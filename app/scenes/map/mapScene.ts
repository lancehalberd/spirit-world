...
export class MapScene implements GameScene {
    ...
    update(state: GameState, interactive: boolean) {
        ...
        const floorKeys = Object.keys(dungeonMaps[state.areaSet?.currentSection?.definition.mapId]?.floors || {});
        ...
        if (floorKeys.length > 1 && wasGameKeyPressed(state, GAME_KEY.UP)) {
            do {
                ...
                && !dungeonMaps[state.areaSet?.currentSection?.definition.mapId]?.floors[floorKeys[this.floorIndex]].sections?.find(section => isSectionExplored(state, section)));
            editingState.selectedSections = [];
