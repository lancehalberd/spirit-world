...
export function getMapOptions(state: GameState, selectedSections: number[]): MenuOption[] {
    ...
    if (!isOverworldZoneKey(state.areaSet?.currentSection.definition.mapId)) {
        mapIds = [state.areaSet?.currentSection.definition.mapId, ...mapIds.filter(mapId => mapId !== state.areaSet?.currentSection.definition.mapId)];
    }
    return [
        {
            label: 'setMapId',
