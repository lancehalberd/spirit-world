...
export function clearTileInOneWorld(area: AreaDefinition, alternateArea: AreaDefinition, zone: Zone, layerKey: string, tx: number, ty: number) {
    ...
    if (area === baseArea) {
        ...
        // from the base world.
        const baseLayer = getOrAddLayer(layerKey, area, alternateArea, zone);
        const childLayer = getOrAddLayer(layerKey, alternateArea, area, zone);
        ...
        const childLayer = getOrAddLayer(layerKey, area, alternateArea, zone);
        childLayer.grid.tiles[ty] = childLayer.grid.tiles[ty] || [];
