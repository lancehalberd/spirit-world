...
export function canMoveRight(
    ...
    for (; i <= maxLoops; i++) {
        ...
        DETERMINE_FLAGS: {
            ...
            if (atx === tx - 1) {
                ...
                if (tileBehaviors?.ledges?.left === false || leftOfPixelBehaviors?.ledges?.right === true) {
                    isAbove = true;
                    break DETERMINE_FLAGS;
                }
            }
            ...
            if (atx === tx - 1) {
                const leftOfPixelBehaviors = area.behaviorGrid[ty]?.[tx - 1];
                // anchor and tile are aligned in the same column.
                if (tileBehaviors?.ledges?.left === true || leftOfPixelBehaviors?.ledges?.right === false) {
                    isUnder = true;
                    break DETERMINE_FLAGS;
                }
            }
        ...
        let blocked = false;
        const result = isMovementBlocked(state, area, tileBehaviors, x, y, isAbove, isUnder, movementProperties, 'right');
        if (result) {
            blocked = true;
            if (result.object) {
    ...
    if (i >= maxLoops) {
        console.log('canMoveRight: did not finish movement checks');
    }
