...
export class LaserBeam implements EffectInstance, Props {
	...
	baseradius = this.props.baseradius ?? 6;
...
export class SentryBotLaser implements EffectInstance, Props {
    area: AreaInstance = null;
    isEffect = <const>true;
    isEnemyAttack = true;
    frame: Frame;
	...
	baseradius = this.props.baseradius ?? 3;
    delay = this.props.delay;
    duration = this.props.duration ?? 200;
    ignoreWalls = this.props.ignoreWalls;
    totalTellDuration = this.props.tellDuration ?? 0;
    ...
    render(context: CanvasRenderingContext2D, state: GameState) {
        ...
        const p = (this.animationTime - this.duration) / FADE_DURATION;
        const alpha = 1 - Math.max(0, Math.min(1, p));
        drawSingleColorLaser(context, padRay(this.getHitRay(state), sentryPadding), alpha);
    }
    renderShadow(context: CanvasRenderingContext2D, state: GameState) {
       if (this.delay > 0) {
           return;
       ...
       if (this.tellDuration > 0) {
           const ray = padRay(this.getHitRay(state), sentryPadding);
           renderDamageWarning(context, {
                ray,
                duration: this.totalTellDuration,
                time: this.totalTellDuration - this.tellDuration,
...
export function drawSingleColorLaser(context: CanvasRenderingContext2D, ray: Ray, alpha: number) {
    context.save();
        context.globalAlpha = alpha;
        context.translate(ray.x1, ray.y1);
        const dx = ray.x2 - ray.x1;
		...
		theta += Random.element(laserJitter);
        context.rotate(theta);
		//console.log(ray.r);
		
        // Pick a color. If this is too slow, we'll find a way to pick a color every N ticks or frames.
		const randomColor = Random.element(colorArray);
